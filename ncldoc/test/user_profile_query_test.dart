  ...
  group('NCL User Profile Query Evaluation Tests', () {
    test('evaluates profile query with numeric comparison operators', () {
      ...
      final profileNe = NCLUserProfile(
        id: 'pNe',
        query: {'attribute': 'age', 'comparator': 'neq', 'value': '15'},
      );
    ...
    test(
      'registers profiles in NCLUsers and evaluates profiles against all registered users',
      () {
    ...
    test(
      'supports allProfiles, removeProfile, and clearing profiles on NCLUsers.clear()',
      () {
        final users = NCLUsers();
    ...
    test(
      'evaluates nested AND/OR rules, string comparisons, and src property in NCLUserProfile',
      () {
        final user = NCLUserData(
          id: 'u1',
          name: 'Dave',
        ...
        expect(
          profileNested.src,
          equals('http://example.com/profiles/pNested.xml'),
        );
    ...
    test(
      'evaluates profile queries with uppercase AND/OR keys, empty rules, missing attributes, and dynamic user updates',
      () {
        final user = NCLUserData(
          id: 'u1',
          name: 'Eve',
    ...
    test(
      ...
      () {
        ...
        expect(
          users.evaluateProfileForUser('nonexistentProfile', 'u1'),
          isFalse,
        );
    ...
    test(
      'supports user property removal and clearing across NCLUserData and NCLUsers',
      () {
        final user = NCLUserData(
          id: 'u1',
          name: 'User 1',
    ...
    test(
      'evaluates profile query with built-in id and name attributes and exports user data',
      () {
        final users = NCLUsers();
        final u1 = NCLUserData(
          id: 'usr_001',
          name: 'Alice Smith',
          initialProperties: {'role': 'admin'},
        );
        final u2 = NCLUserData(
          id: 'usr_002',
          name: 'Bob Jones',
          initialProperties: {'role': 'user'},
        );
        users.registerUser(u1);
        users.registerUser(u2);
        ...
        final pId = NCLUserProfile(
          id: 'pId',
          query: {'attribute': 'id', 'comparator': 'eq', 'value': 'usr_001'},
        );
        final pName = NCLUserProfile(
          id: 'pName',
          query: {
            'attribute': 'name',
            'comparator': 'eq',
    ...
    test('evaluates profiles and tests findProfilesForUser', () {
      ...
      final pContains = NCLUserProfile(
        id: 'p1',
        query: {
          'attribute': 'email',
          'comparator': 'eq',
      ...
      final pStartsWith = NCLUserProfile(
        id: 'p2',
        query: {
          'attribute': 'city',
          'comparator': 'eq',
      ...
      final pEndsWith = NCLUserProfile(
        id: 'p3',
        query: {
          'attribute': 'city',
          'comparator': 'eq',
      ...
      final pInList = NCLUserProfile(
        id: 'p4',
        query: {'attribute': 'category', 'comparator': 'eq', 'value': 'vip'},
      );
      final pInCsv = NCLUserProfile(
        id: 'p5',
        query: {'attribute': 'category', 'comparator': 'eq', 'value': 'vip'},
      );
      final pNoMatch = NCLUserProfile(
        id: 'p6',
        query: {'attribute': 'category', 'comparator': 'eq', 'value': 'basic'},
      );
